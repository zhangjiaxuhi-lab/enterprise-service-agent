...
用法::
    ...
    # 2) 再运行本脚本
    python src/api/test_sse.py                       # 跑三个内置场景
    python src/api/test_sse.py "你们的退款政策是什么？"   # 自定义单条消息
    python src/api/test_sse.py --url http://127.0.0.1:8000 --thread t-1 "你好"
...
def main() -> int:
    ...
    for index, (label, message) in enumerate(scenarios, start=1):
        ...
        assert "done" in kinds, "缺少 done 结束事件"
        if label == "场景2 槽位缺失反问":
            assert "tool_start" not in kinds, "槽位缺失时不应触发任何工具调用"
            print("  ✅ 校验通过：未触发工具，符合槽位填充约束")
        elif label == "场景3 参数齐备提交":
            assert any(
                event.get("type") == "tool_start" and event.get("tool") == "submit_ticket"
                for event in events
            ), "参数齐备时应调用 submit_ticket"
            print("  ✅ 校验通过：已调用 submit_ticket")
